...
#include "schedulers/cilkplus.h"  // IWYU pragma: keep, export

#elif defined(PARLAY_OPENCILK)
#include "schedulers/opencilk.h"  // IWYU pragma: keep, export

// OpenMP Scheduler
#elif defined(PARLAY_OPENMP)
#include "schedulers/omp.h"  // IWYU pragma: keep, export

// Intel's Thread Building Blocks Scheduler
#elif defined(PARLAY_TBB)
#include "schedulers/tbb.h"  // IWYU pragma: keep, export

// No Parallelism
#elif defined(PARLAY_SEQUENTIAL)
#include "schedulers/sequential.h"  // IWYU pragma: keep, export
...
inline void parallel_for(size_t start, size_t end, F f, long granularity,
                         bool conservative) {
  if (end > start)
    internal::get_default_scheduler().parfor(start, end, f, (size_t)granularity,
                                             conservative);
}
